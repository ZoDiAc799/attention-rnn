...
class MyGRUCell(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.Wxr = nn.Linear(self.input_size,self.hidden_size,bias=False)
        self.Whr = nn.Linear(self.hidden_size,self.hidden_size)
        self.Wxz = nn.Linear(self.input_size,self.hidden_size,bias=False)
        self.Whz = nn.Linear(self.hidden_size,self.hidden_size)
        self.Wxg = nn.Linear(self.input_size,self.hidden_size,bias=False)
    ...
    def forward(self, x, h_prev):
        ...
        # ------------
        z = torch.sigmoid(self.Wxr(x)+self.Whr(h_prev))
        r = torch.sigmoid(self.Wxz(x)+self.Whz(h_prev))
        g = torch.tanh(self.Wxg(x)+r*self.Whg(h_prev))
...
class Attention(nn.Module):
    ...
    def forward(self, hidden, annotations):
        ...
        attention_net_output = self.attention_network(concat)
...
class AttentionDecoder(nn.Module):
    ...
    def forward(self, x, h_prev, annotations):
        ...
        context = torch.sum(annotations*attention_weights,dim=1)
